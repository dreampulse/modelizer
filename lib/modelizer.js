...
//    this.delete = function() {};
    ...
    obj.save = function() {
      ...
      for (var i=0; i<self.attrs.length; i++) {
        assert(obj.hasOwnProperty(self.attrs[i].name) == true, "attribute not specifiyed");
        try {
  ...
  this.useFiltered = {
    ...
    find : function(search, req) {
      ...
      if (filter === false) {  // deny all
        var deferred = Q.defer();
        deferred.reject(new Error("Access denied!"));
        return deferred.promise;
      }
...
//      }
  ...
  this.processSchema = function(schema) {
    assert(typeof schema == 'object', "Schema defintion has to be a Hash");

    for (var entry in schema) {
      var value = schema[entry];
      //assert(typeof value == 'object'value.hasOwnProperty("what"), "definition error in " + entry);

      if (Array.isArray(value)) {  // it is a attrArray
        var attrArrayModel = new Model(entry, value);
