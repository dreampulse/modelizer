...
var Model = function(modelName, schema) {
  ...
  this.createObject = function(initValues) {
  ...
  this.use = {
    ...
    save : function(obj) {
      ...
      self.collection.save(obj, function(err, doc) {
        ...
        deferred.resolve(doc);  // todo sollte doch obj zurück gegeben werden
      });
...
Model.AngularConnector = function(connectionUrl) {
  ...
  // serialize doc
  var transform = function(model, doc, method) {
    ...
    // pack attrObjs
    for (var i in model.attrObjs) {
      transform(model.attrObjs[i], doc[i], method);
    }

    // copy attrArrays
    for (var i in model.attrArrays) {
      for (var j in doc[i]) {
        transform(model.attrArrays[i], doc[i][j], method);
      }
    }
...
Model.Attr.Types = {
  ...
  date : function(value, action) {
    ...
    if (Date.parse(value).toString() == "NaN" && value !== undefined && value !== null) {
