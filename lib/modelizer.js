  ...
  this._addStore = function(obj) {
    ...
    var prepareSave = function(obj, model) {
      ...
      for (var i=0; i<model.attrs.length; i++) {
        assert(obj.hasOwnProperty(model.attrs[i].name), "Attribute '"+ model.attrs[i].name +"'not specified in model '"+model.modelName+"'");
        ...
        try {
          for (var j=0; j<model.attrs[i].filters.length; j++) {
            model.attrs[i].filters[j](doc, model.attrs[i].name);  // call filter
          }
        } catch (err) {
    ...
    obj.save = function() {
      ...
      return self.use.save(doc)  // store the document
