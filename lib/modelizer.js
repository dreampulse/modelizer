...
var Model = function(modelName, schema) {
  ...
  this.attr = function(attrName) {
    ...
    this.attrs.push({name : attrName, filters : filters});
  ...
  this.factorys = {};
  this.factory = function(factoryName) {
    this.factorys[factoryName] = factoryName;
    this[factoryName] = function(params) {
      return this.use.callOp(factoryName, params, null);
    }
  ...
  this.factoryImpls = {};
  this.factoryImpl = function(factoryName, fnImpl) {
  ...
  this.serve = function() {
    ...
    // REST - GET find
    this.app.post('/' + modelName + '/find', function(req, res){
     res.setHeader('Content-Type', 'application/json');
      self.useFiltered.find(req.body, req)
        .then(function(docs) {
          res.send(JSON.stringify(docs, null, 2));
        })
        .fail(function (err) {
    ...
    this.app.get('/' + modelName + '/:id', function(req, res) {
      res.setHeader('Content-Type', 'application/json');
      self.useFiltered.get(req.params.id, req)
        .then(function(doc) {
    ...
    this.app.put('/' + modelName, function(req, res) {
      ...
      self.useFiltered.save(req.body, req)
        .then(function(doc) {
          res.send(JSON.stringify(doc, null, 2));
        })
        .fail(function (err) {
    ...
    this.app.del('/' + modelName + '/:id', function(req, res) {
      res.setHeader('Content-Type', 'application/json');
      self.useFiltered.remove(req.params.id, req)
        .then(function() {
          res.send(200);
        })
        .fail(function (err) {
  ...
  this.createArrayReferenceElement = function(refModel, obj, arrayName) {
  ...
  this.createArrayElement = function(arrayModel, obj, arrayName) {
...
//    this.delete = function() {};
    ...
    obj.save = function() {
      ...
      for (var i=0; i<self.attrs.length; i++) {
        assert(obj.hasOwnProperty(self.attrs[i].name) == true, "attribute not specified");
        
        doc[self.attrs[i].name] = obj[self.attrs[i].name];  // copy attribute

        // TODO: filter api so verändern, dass ich das auf das kopiere obj anweden kann

        // TODO: funktionier das eigentlich für Arrays und Objects? -> testen

        try {
          for (var j=0; j<self.attrs[i].filters.length; j++) {
            self.attrs[i].filters[j](doc, self.attrs[i].name);  // call filter
          }
        } catch (err) {
          err.message = "Can't save '"+self.attrs[i].name+"' " + err.message;
          deferred.reject(err);
          return deferred.promise;
        };

        obj[self.attrs[i].name] = doc[self.attrs[i].name];  // copy back to obj
      }
      ...
      for (var i in self.attrRefs) {
        ...
        if (doc[i].ref !== undefined && doc[i].ref()._id !== undefined) {  // wenn das referenzierte Object existiert und bereits gespeichert wurde
          doc[i]._reference = doc[i].ref()._id;  // die reference id mit speichern
        }
  ...
  this._getReadFilter = function(req) {
    var res = {};
    for (var i=0; i<self.readFilters.length; i++) {  // alle filter
      var filter = self.readFilters[i](req);
  ...
  // create the filter hash (mongo search string)
  this._getWriteFilter = function(obj, req) {
    var res = {};
    for (var i=0; i<self.writeFilters.length; i++) {  // alle filter
      var filter = self.writeFilters[i](obj, req);
      if (filter === false) return false;  // deny all filter
      if (filter === true) continue;  // ignore filter
...
//  };
  ...
  this.useFiltered = {
    ...
    get : function(id_str, req) {
      ...
      return self.useFiltered.find({_id:id}, req)
        .then(function (docs) {
          if (docs.length != 1) {
            var deferred = Q.defer();
    ...
    find : function(search, req) {
      var filter = self._getReadFilter(req);
    ...
    all : function(req) {
      return self.useFiltered.find({}, req);
    },

    save : function(obj, req) {
...
//      }
  ...
  this.use = {
    ...
    save : function(obj) {
      var deferred = Q.defer();
      assert(self.collection != undefined, "connection no set for " + self.modelName);
  ...
  this.processSchema = function(schema) {
    assert(typeof schema == 'object', "Error in Schema definition (has to be a Hash)");
    ...
    return {
      find : function(search, callback) {
        if (isEmptyObject(search)) {
          ...
          ajaxCall('POST', connectionUrl + theModel.modelName + '/find', search, callback);
        }
      },
...
Model.Attr = function() {
  ...
  return {
    '_what' : 'attr',
    filter: function(obj, name) {
      for (var i in filters) {
        filters[i](obj, name);  // call all filters
      }
    }
  };
};
...
Model.Attr.default = function(def) {
  return function(obj, name) {
    if (obj[name] == undefined) obj[name] = def;
  }
};

Model.Attr.Types = {
  // define a string type
  string : function(obj, name) {
    var value = obj[name];
  ...
  number : function(obj, name) {
    var value = obj[name];
    if (typeof value != 'number' && value !== undefined && value !== null) {
      throw new Error("'"+value+"' is not a number");
    }
  },
  boolean : function(obj, name) {
    var value = obj[name];
